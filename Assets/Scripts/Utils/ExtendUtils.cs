...
namespace Utils
{
    public static class ExtendUtils
    {
        ...
        {
            var parent = transform.Find(Constant.BUTTON_PARENT_NAME);
            if (parent == null)
            {
              Debug.LogError("can not find ButtonParent name:"+ Constant.BUTTON_PARENT_NAME);
                return null;
            }
            else
            {
        ...
        {
            var buttonTrans = transform.Find(Constant.BUTTON_PARENT_NAME + "/" + btnName);
            if (buttonTrans == null)
            {
